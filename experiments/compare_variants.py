...
def plot_results(num_failures: int, graph_size: int, algorithm: str) -> None:
    ...
    # 3. Total Query Cost
    # 4. Total Task Cost
    # Where we have 4 group of 3 bars each (one for each variant).
    metrics = [
        "total_timesteps",
        "total_executions",
        "total_queries",
    ...
    variants = ["greedy", "balanced", "conservative"]
    variant_results = [greedy_results, balanced_results, conservative_results]
...
def main(failures_list: list, graph_size: int, algorithm: str) -> None:
    """Main function to plot results for a fixed number of failures, graph
    size, and algorithm."""
    for num_failures in failures_list:
        plot_results(num_failures, graph_size, algorithm)
...
if __name__ == "__main__":
    ...
    main(args.failures_list, args.graph_size, args.algorithm)
