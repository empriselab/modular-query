...
## Outcome: for all 5 IVs, we generate two rows of plots.
## First row: compares module selectors.
## Second row: compares querying algorithms.
## Each row has 5 plots in it (one for each metric.)
    ...
    for i, row in enumerate(rows):
        for j, column in enumerate(columns):            
            ...
            if row == "module_selectors":
                ...
                if iv == "num_modules":
                    individual_plot_num_modules(df, fixed_variables, metric, iv, order_dict[iv], ax, graph_size)
                else:   
                    individual_plot(df, fixed_variables, metric, iv, order_dict[iv], ax, graph_size)
                ...
                if iv == "num_modules":
                    if metric == "execution_time_total":
                        ...
                        ymax=None
                    individual_plot_num_modules_fixed_moduleselector(df, fixed_variables, metric, iv, order_dict, ax, graph_size, fixed_module_selector, ymax=ymax)
                else:   
                    individual_plot_fixed_moduleselector(df, fixed_variables, metric, iv, order_dict, ax, graph_size, fixed_module_selector)
